...
crop_GDD <- readr::read_csv(here::here("inst/crops.csv"),
...
gdd.recons <- 
  purrr::map_chr(1:nrow(crop_GDD),
                 function(n){
                   ...
                   if(
                     file.exists(paste0(figures,"cultivar_niches/",cultivar,".pdf")) & 
                     file.exists(paste0(figures,"cultivar_niches/",cultivar,".mp4"))
...
dir.create(paste0(figures,"crop_niches/"),
           recursive = TRUE,
           showWarnings = FALSE)
...
for(n in 1:nrow(crops)){
  ...
  rasts <- readr::read_rds(paste0(derived_data,"recons/all_",crop,".rds")) %>%
    purrr::map(function(x){
      x %>%
  ...
  if(!file.exists(paste0(figures,"crop_niches/all_",crop,".pdf")))
    gutaker2020:::space_time_plot(the_brick = rasts$Z,
                                  the_brick_lower = rasts$Z_Lower,
                                  the_brick_upper = rasts$Z_Upper,
  ...
  if(!file.exists(paste0(figures,"crop_niches/all_",crop,".mp4"))){
    gutaker2020:::space_time_video(the_brick = rasts$Z,
                                   the_brick_lower = rasts$Z_Lower,
                                   the_brick_upper = rasts$Z_Upper,
...
p <- facet_niche(crops = crops$crop,
                 years = c(4410, 3550))

cairo_pdf(paste0(figures,"Figure_2_facet_niche.pdf"), width = 7.2, height = 10.3)
print({
  p +
...
ggplot2::ggsave(paste0(figures,"japan_thailand_compare.pdf"),
                width = 6.5,
                height = 6)
...
ggplot2::ggsave(paste0(figures,"japan_thailand_GDD_compare.pdf"),
                width = 6.5,
                height = 6.5/1.618)
